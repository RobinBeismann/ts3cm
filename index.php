<?php
	...
	if(mysql_num_rows($result)==1)
	{
		...
		{
			mysql_query("DELETE FROM `ts3cm_assoc` WHERE (`client_id`='".$clientdbid."') AND (`channel_id`='".$channel_id."')");
	...
	else 
	{
		...
		$srv->clientMove($clientcurid,$channel_id,null);
		$srv->clientSetChannelGroup($clientdbid,$channel_id,5);
		mysql_query("INSERT INTO `ts3cm_assoc` (`client_id`, `channel_id`) VALUES ('" . $clientdbid . "', '".$channel_id."')");
	}
	...
	function getClient($_clip){
		...
		foreach($srv->clientList() as $ts3_Client)
		{
			...
			$ip = $clarray["client_lastip"]->toString ();
			if(($ip == $_clip) ){
